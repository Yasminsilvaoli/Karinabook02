document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        if (mobileMenuBtn && navLinks) {
            const toggleMenu = () => {
                console.log('KarinaBook: Toggling Mobile Menu');
            ...
            // All links inside the nav drawer should close the menu when clicked
            navLinks.querySelectorAll('a').forEach(link => {
                link.addEventListener('click', (e) => {
                    // Specific logic for controls (account, cart, theme)
                    if (link.id === 'mobile-user-link') {
                        e.preventDefault();
                        toggleMenu();
                        document.getElementById('login-modal').classList.add('active');
                    } else if (link.id === 'mobile-cart-link') {
                        e.preventDefault();
                        toggleMenu();
                        document.getElementById('cart-modal').classList.add('active');
                    } else if (link.id === 'mobile-theme-link') {
                        e.preventDefault();
                        const current = document.documentElement.getAttribute('data-theme');
                        const target = current === 'dark' ? 'light' : 'dark';
                        document.documentElement.setAttribute('data-theme', target);
            ...
            const handleMobileSearch = () => {
                searchQuery = searchInputMobile.value;
                renderProducts();
                if (searchQuery) {
                    if (navLinks.classList.contains('active')) toggleMenu();
            ...
            if (searchBtnMobile) searchBtnMobile.addEventListener('click', handleMobileSearch);
            if (searchInputMobile) searchInputMobile.addEventListener('keyup', (e) => { if(e.key === 'Enter') handleMobileSearch(); });
        }
    }
